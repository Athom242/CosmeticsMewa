...
import {
  cacheListProduct,
  cacheRecentProduct,
} from "../component/ActionMethod/contextData";
...
const ProducItemtInfoError = function ({ idValue }) {
  ...
  return (
    <div className="productInfoError">
      <p>
        la valuer de id = <span className="idProduct">{`${idValue}`}</span>
      </p>
      <p>
        ce document n'a pas pu etre trouver dans la base de donnée de notre
        catalogue
      </p>
...
function productItemDetailGet(idObjectInfo) {
  ...
  return dataItem.products.filter(
    (index) => index.idProductType === idObjectInfo
  );
}
      ...
      <div className="sideimage">
        {imageObjectList.map((index, value) => {
          return (
            <a
              href="#"
              onMouseEnter={handleViewImageOnMain(value)}
              onMouseLeave={handleViewImageOnDownMain}
              className="imgLink"
            >
...
function ButtonCountChange({
  value = 0,
  handleChangeValue = (idChoice) => {
    console.log(idChoice);
  },
}) {
  return (
    <div className="buttonCount">
      <span className="buttonChangeValue" onClick={handleChangeValue(0)}>
        -
      </span>
      <span className="currentValue">{value}</span>
      <span className="buttonChangeValue" onClick={handleChangeValue(1)}>
        +
      </span>
    </div>
  );
...
function PurchaseLink({ idProduct, handleProductShop }) {
  ...
  return <Link onClick={handleProductShop}>Purchase</Link>;
}
        ...
        <div className="productHeaderInfo">
          ...
          <div className="price">
            <h3>
              ...
              <span>{productDetail.price.current}</span>
            </h3>
          </div>
        </div>

        <div className="productDescr">
          <p>{productDetail.descr.long}</p>
        </div>

        <div className="productFeatureShop">
          <ButtonCountChange value={0} handleChangeValue={handleChangeValue} />
          <PurchaseLink
            idProduct={productDetail.idProductType}
            handleProductShop={handleProductShop()}
          />
        </div>
...
export function AddCommentSection({ profile }) {
...
function ProductFeatureInfo({ productDetail }) {
    ...
    return (
      <div className="containt productInfoFeature productInfoFeatureReview active">
        <div className="titleFeature">
          <h2>
            <span className="countComment">{countComment}</span> REVIEW FOR{" "}
            {nameProduct}
          </h2>
        </div>
  ...
  const headerControl = [
    {
      link: "Description",
      container: <ProductInfoFeaturedescr productDetail={productDetail} />,
    },
    {
      link: "Additional Information",
      container: <ProductInfoFeatureAddInfo productDetail={productDetail} />,
    },
    {
      link: "Review",
      container: <ProductInfoFeatureReview productDetail={productDetail} />,
    },
  ];
      ...
      <div className="headerControl">
        <ul className="headerControlList">
          {headerControl.map((value, index) => {
            return (
              <li>
                <a href="#" onClick={handleChangeViewProduct(index)}>
                  {value.link}
                </a>
              </li>
            );
...
const ProductInfoRelated = ({}) => {
  ...
  return (
    <div className="containt productInfoRelated">Bonjour tout le monde</div>
  );
};
